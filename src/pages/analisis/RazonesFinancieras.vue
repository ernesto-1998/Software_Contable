...
let razon = ref(null);
let periods = [];
let razones = ["Razones de Liquidez", "Razones de Actividad", "Razones de Deuda", "Razones de Rendimiento"];
let title = ref("");
...
const activarRazonesLiquidez = (año) => {
    ...
    columns.value = ["Año", "Razón Circulante", "Razón Rápida", "Capital de Trabajo", "Nivel de Dependencia"];
    rows.value.push([año , datos.razon_circulante, datos.razon_rapida, datos.capital_trabajo, datos.nivel_dependencia]);
}

const activarRazonesActividad = (año) => {
    title.value = razon.value;
    const datos = obtenerDatosRazones(año);
    columns.value = ["Año", "Rotación de Inventarios", "Rotación de Cuentas por Cobrar", "Número de días Inventario", "Ciclo Operacional", "Periodo Promedio de Cobro", "Periodo Promedio de Pago", "Rotacion de Activo Total", "Rotación de Activo Operacional"];
    rows.value.push([año, datos.rotacion_inventarios, datos.rotacion_cuentas_por_cobrar, datos.numero_dias_inventario, datos.ciclo_operacional, datos.periodo_promedio_cobro, datos.periodo_promedio_pago, datos.rotacion_activo_total, datos.rotacion_activo_operacional]);
}
...
const activarRazonesDeuda = (año) => {
    ...
    rows.value.push([año , datos.razon_endeudamiento, datos.razon_cargos_interes_fijo]);
}

const activarRazonesRendimiento = (año) => {
    title.value = razon.value;
    const datos = obtenerDatosRazones(año);
    columns.value = ["Año", "ROA", "ROE", "Margen Utilidad Bruta", "Margen Utilidad Operativa", "Margen Utilidad Neta"];
    rows.value.push([año , datos.roa, datos.roe, datos.margen_utilidad_bruta, datos.margen_utilidad_operativa, datos.margen_utilidad_neta]);
...
const obtenerDatosRazones = (año) => {
    ...
    const rotacion_cuentas_por_cobrar = razones_actividad.rotacion_cuentas_por_cobrar(cuentas_por_cobrar, ventas).toFixed(2);
    const rotacion_inventarios = razones_actividad.rotacion_inventarios(costo_de_ventas, inventarios).toFixed(2);
    const numero_dias_inventario = razones_actividad.numero_dias_inventario(inventarios, costo_de_ventas).toFixed(2);
    const ciclo_operacional = razones_actividad.ciclo_operacional(cuentas_por_cobrar, ventas, inventarios, costo_de_ventas).toFixed(2);
    const periodo_promedio_cobro = razones_actividad.periodo_promedio_cobro(cuentas_por_cobrar, ventas).toFixed(2);
    const periodo_promedio_pago = razones_actividad.periodo_promedio_pago(cuentas_por_pagar, costo_de_ventas).toFixed(2);
    const rotacion_activo_total = razones_actividad.rotacion_activo_total(ventas, activo).toFixed(2);
    const rotacion_activo_operacional = razones_actividad.rotacion_activo_operacional(ventas, cuentas_por_cobrar, inventarios, activo_no_corriente, total_depreciacion, efectivo).toFixed(2);

    // Razones de Deuda

    const razon_endeudamiento = razones_deuda.razon_endeudamiento(pasivo, activo).toFixed(2);
    const razon_cargos_interes_fijo = razones_deuda.razon_cargos_interes_fijo(utilidad_antes_impuestos, 1).toFixed(2);
    ...
    const margen_utilidad_bruta = razones_rendimiento.margen_utilidad_bruta(utilidad_bruta, ventas).toFixed(2);
    const margen_utilidad_operativa = razones_rendimiento.margen_utilidad_operativa(utilidad_operativa, ventas).toFixed(2);
    const margen_utilidad_neta = razones_rendimiento.margen_utilidad_neta(utilidad_neta, ventas).toFixed(2);
